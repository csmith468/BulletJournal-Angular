...
GO
CREATE TABLE [checklist].[physical](
...
GO
ALTER TABLE [checklist].[physical] ADD PRIMARY KEY CLUSTERED 
(
	[id] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
GO
ALTER TABLE [checklist].[physical]  WITH CHECK ADD  CONSTRAINT [FK_physicalUser] FOREIGN KEY([userID])
REFERENCES [app].[user] ([userID])
GO
ALTER TABLE [checklist].[physical] CHECK CONSTRAINT [FK_physicalUser]
GO
