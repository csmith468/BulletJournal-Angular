...
GO
CREATE TABLE [checklist].[wellbeing](
...
GO
ALTER TABLE [checklist].[wellbeing] ADD PRIMARY KEY CLUSTERED 
(
	[id] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
GO
ALTER TABLE [checklist].[wellbeing]  WITH CHECK ADD  CONSTRAINT [FK_wellbeingUser] FOREIGN KEY([userID])
REFERENCES [app].[user] ([userID])
GO
ALTER TABLE [checklist].[wellbeing] CHECK CONSTRAINT [FK_wellbeingUser]
GO
