ALTER FUNCTION [bja].[getUTCInTimezone](
    @TimezoneID int,
...
BEGIN
    ...
    DECLARE @TimezoneBiasID int = (SELECT TimezoneBiasID FROM bja.timezone WHERE TimezoneID = @TimezoneID);
    DECLARE @StandardBias int = (SELECT StandardBias FROM bja.timezone WHERE TimezoneID = @TimezoneID);
    DECLARE @TransitionBias int = (SELECT TransitionBias FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);

    -- parts of date for normal section of daylight savings
    DECLARE @NormalDayOfWeek int = (SELECT TOP(1) NormalDayOfWeek FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @NormalDayOfWeekOccurrence int = (SELECT TOP(1) NormalDayOfWeekOccurrence FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @NormalMonth int = (SELECT TOP(1) NormalMonth FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @NormalTimeOfDay time = (SELECT TOP(1) NormalTimeOfDay FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);

    -- parts of date for transition section of daylight savings
    DECLARE @TransitionDayOfWeek int = (SELECT TOP(1) TransitionDayOfWeek FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @TransitionDayOfWeekOccurrence int = (SELECT TOP(1) TransitionDayOfWeekOccurrence FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @TransitionMonth int = (SELECT TOP(1) TransitionMonth FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @TransitionTimeOfDay time = (SELECT TOP(1) TransitionTimeOfDay FROM bja.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    ...
    RETURN CASE WHEN @DatetimeInTimezone BETWEEN @AddDatetime AND @SubtractDatetime
        THEN @DatetimeInTimezone
