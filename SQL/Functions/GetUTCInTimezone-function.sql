...
BEGIN
    ...
    DECLARE @TimezoneBiasID int = (SELECT TimezoneBiasID FROM dbo.timezone WHERE TimezoneID = @TimezoneID);
    DECLARE @StandardBias int = (SELECT StandardBias FROM dbo.timezone WHERE TimezoneID = @TimezoneID);
    DECLARE @TransitionBias int = (SELECT TransitionBias FROM dbo.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);

    -- parts of date for normal section of daylight savings
    DECLARE @NormalDayOfWeek int = (SELECT TOP(1) NormalDayOfWeek FROM dbo.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @NormalDayOfWeekOccurrence int = (SELECT TOP(1) NormalDayOfWeekOccurrence FROM dbo.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @NormalMonth int = (SELECT TOP(1) NormalMonth FROM dbo.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @NormalTimeOfDay time = (SELECT TOP(1) NormalTimeOfDay FROM dbo.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);

    -- parts of date for transition section of daylight savings
    DECLARE @TransitionDayOfWeek int = (SELECT TOP(1) TransitionDayOfWeek FROM dbo.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @TransitionDayOfWeekOccurrence int = (SELECT TOP(1) TransitionDayOfWeekOccurrence FROM dbo.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
    DECLARE @TransitionMonth int = (SELECT TOP(1) TransitionMonth FROM dbo.timezoneBias WHERE TimezoneBiasID = @TimezoneBiasID);
